...
function getKlines(symbol, callback) {
  ...
  try {
    const params = {
      symbol,       // Cặp giao dịch, ví dụ: BONKUSDT
...
function buyAtLowestPrice() {
  getKlines('BONKUSDT', (err, prices) => {
    if (err) return console.error('Lỗi khi lấy dữ liệu giá:', err);
    const quantity = roundDownToFourDecimalPlaces(AMOUNT_USDT / prices.low, 0);
    const price = roundDownToFourDecimalPlaces(prices?.low, 8);
    console.log('Đặt lệnh mua BONK:', {quantity, price});
    sendRequest(
      'POST',
      '/api/v3/order',
      {
      ...
      (err, response) => {
        if (err) return sendTelegramMessage('Mua BONK Error: ' + JSON.stringify(err)).then();
        if (response?.orderId) {
          const message = getMessageData({
            symbol: 'BONKUSDT', quantity, price, amount: quantity * price
          }, true);
          ...
          const msg = getMessageData({
            symbol: 'BONKUSDT', quantity, price, message, amount: quantity * price
          }, true);
...
function sellAtHighestPrice() {
  getKlines('BONKUSDT', (err, prices) => {
    if (err) return console.error('Lỗi khi lấy dữ liệu giá BONK:', err);

    console.log(`Giá cao nhất để bán BONK: ${prices.high}`);

    // Lấy số dư BONK hiện tại
    sendRequest('GET', '/api/v3/account', {}, (err, data) => {
      ...
      if (BONKBalance >= 0.0001) {
        ...
        sendRequest(
          ...
          (err, response) => {
            if (err) return sendTelegramMessage('Bán BONK Error: ' + JSON.stringify(err)).then();
            if (response?.orderId) {
              const message = getMessageData({
                symbol: 'BONKUSDT', quantity, price, amount: quantity * price
              }, false);
        ...
        const message = 'Không có BONK để bán.';
        sendTelegramMessage(message).then();
      }
    });
  });
...
function BONKSchedulerCronJob() {
  ...
  schedule.scheduleJob(cronJobStrLogTime, function () {
    console.log('Current time BONK Run CronJob:', moment().format('YYYY-MM-DD HH:mm:ss'));
  });

  let cronJobStrBUY2 = isUTC ? '0 10 * * *' : '0 17 * * *';
  schedule.scheduleJob(cronJobStrBUY2, () => {
    console.log('Mua BONK dựa trên giá thấp nhất trước 5:00PM...', API_KEY);
    buyAtLowestPrice();
  });

  let cronJobStrSELL2 = isUTC ? '0 15 * * *' : '0 22 * * *';
  schedule.scheduleJob(cronJobStrSELL2, () => {
    console.log('Bán BONK dựa trên giá cao nhất trước 10:00PM...');
    sellAtHighestPrice();
  });
