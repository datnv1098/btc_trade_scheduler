...
function getKlines(symbol, callback) {
  ...
  try {
    const params = {
      symbol,       // Cặp giao dịch, ví dụ: SOLUSDT
...
function buyAtLowestPrice() {
  getKlines('SOLUSDT', (err, prices) => {
    ...
    console.log('Đặt lệnh mua SOL:', {quantity, price});
    sendRequest(
      'POST',
      '/api/v3/order',
      {
      ...
      (err, response) => {
        if (err) return sendTelegramMessage('Mua SOL Error: ' + JSON.stringify(err)).then();
        if (response?.orderId) {
          const message = getMessageData({
            symbol: 'SOLUSDT', quantity, price, amount: quantity * price
          }, true);
          ...
          const msg = getMessageData({
            symbol: 'SOLUSDT', quantity, price, message, amount: quantity * price
          }, true);
...
function sellAtHighestPrice() {
  getKlines('SOLUSDT', (err, prices) => {
    if (err) return console.error('Lỗi khi lấy dữ liệu giá SOL:', err);

    console.log(`Giá cao nhất để bán SOL: ${prices.high}`);

    // Lấy số dư SOL hiện tại
    sendRequest('GET', '/api/v3/account', {}, (err, data) => {
      ...
      if (SOLBalance >= 0.001) {
        ...
        sendRequest(
          ...
          (err, response) => {
            if (err) return sendTelegramMessage('Bán SOL Error: ' + JSON.stringify(err)).then();
            if (response?.orderId) {
              const message = getMessageData({
                symbol: 'SOLUSDT', quantity, price, amount: quantity * price
              }, false);
        ...
        const message = 'Không có SOL để bán.';
        sendTelegramMessage(message).then();
      }
    });
  });
...
function SOLSchedulerCronJob() {
  ...
  schedule.scheduleJob(cronJobStrLogTime, function () {
    console.log('Current time SOL Run CronJob:', moment().format('YYYY-MM-DD HH:mm:ss'));
  });

  let cronJobStrBUY = isUTC ? '0 22 * * *' : '0 5 * * *';
  schedule.scheduleJob(cronJobStrBUY, () => {
    console.log('Mua SOL dựa trên giá thấp nhất trước 5h sáng...', API_KEY);
    buyAtLowestPrice();
  });

  let cronJobStrSELL = isUTC ? '0 3 * * *' : '0 10 * * *';
  schedule.scheduleJob(cronJobStrSELL, () => {
    console.log('Bán SOL dựa trên giá cao nhất trước 10h sáng...');
    sellAtHighestPrice();
  });

  // let cronJobStrBUY2 = isUTC ? '0 10 * * *' : '0 17 * * *';
  // schedule.scheduleJob(cronJobStrBUY2, () => {
  //   console.log('Mua SOL dựa trên giá thấp nhất trước 5:00PM...', API_KEY);
  //   buyAtLowestPrice();
  // });
  //
  // let cronJobStrSELL2 = isUTC ? '0 15 * * *' : '0 22 * * *';
  // schedule.scheduleJob(cronJobStrSELL2, () => {
  //   console.log('Bán SOL dựa trên giá cao nhất trước 10:00PM...');
  //   sellAtHighestPrice();
  // });
